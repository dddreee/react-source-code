...
function getIteratorFn(maybeIterable) {
  ...
  // 如果有遍历器属性，相当与返回的是 function values(){ [native code] }
...
var emptyObject = {};
// 冻结 emptyObject 无法修改此对象的属性以及desc的配置  Object.freeze: https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Object/freeze
{
...

{
  var printWarning = function (format) {
    ...
    // 循环 arguments 对象，从下标 1 开始遍历，将除了 format 的其他参数传入 args 数组中
    for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }

    var argIndex = 0;
    // %s 是一个占位符？ 用 format 之后的参数依次替换 %s 
 ...
 */
// 一个便捷组件，默认带有浅比较。shouldComponentUpdate
...
function hasValidRef(config) {
  // config 是 props , createElement 传入的 config 
  {
...
// key 和 ref 是两个特殊的属性，给React使用的 https://reactjs.org/warnings/special-props.html
...
// forEachChildren中调用了次方法， mapResult和keyPrefix 为null
 ...
 * @param {!string} nameSoFar Name of the key path so far. 目前为止的 key 路径的名称
 * @param {!function} callback Callback to invoke with each child found. 
 * @param {?*} traverseContext Used to pass information throughout the traversal  用于在遍历过程中传递信息
 * process.
 * @return {!number} The number of children in this subtree.
 */
// 遍历所有的子元素 返回子元素的count, 依次调用callback。 子元素为 null, undefined, boolean, number 时，停止遍历。
function traverseAllChildrenImpl(children, nameSoFar, callback, traverseContext) {
  ...
  /**
   * 当 children 是 null, number, REACT_PORTAL_TYPE 时， invokeCallback 为true
   */
  if (invokeCallback) {
  ...
  if (Array.isArray(children)) {
    ...
    // 如果 children 是可遍历的，有遍历器属性
    var iteratorFn = getIteratorFn(children);
    if (typeof iteratorFn === 'function') {
      // interatorFn: function values(){ [native code] }
      {
    ...
    // iteratorFn 是null 并且 children 是没有遍历器属性的对象
    } else if (type === 'object') {
      var addendum = '';
      {
...
// 遍历 children 通常指定 `props.children`属性，也可能指定其他属性
/**
 ...
 *
 * traverseContext 是一个可选的参数，通过整个遍历传递。它可以用来存储累积或者回调相关的任何其他内容
...
function forEachSingleChild(bookKeeping, child, name) {
  // forEachChildren调用时，bookKeeping 就是传入的 traverseContext, 如果children 不可遍历 child的值就是字符串的 `children`
  var func = bookKeeping.func,
      context = bookKeeping.context;

  func.call(context, child, bookKeeping.count++);

  // child, bookKeeping.count 就是传入的参数
  /**
   * React.Children.forEach(children, function(child, bookKeeping.count){
   *  ...
   * })
   */
}
 ...
 * @param {*} forEachContext Context for forEachContext. 可选参数？ 文档中只传了两个参数
 */
// 遍历 props.children React.Children.forEach
function forEachChildren(children, forEachFunc, forEachContext) {
   ...
   * }
   */
  // 调用的 traverseAllChildrenImpl(children, '', callback = forEachSingleChild, traverseContext)
  traverseAllChildren(children, forEachSingleChild, traverseContext); 
  // 往 traverseContextPool 添加 traverseContext, 并且将traverseContext 中的属性值都赋值 null ??
  releaseTraverseContext(traverseContext);
}

// React.Children.map 中调用
function mapSingleChildIntoContext(bookKeeping, child, childKey) {
  ...
  // mappedChild 是 map方法中 return 的值。
  ...
  if (Array.isArray(mappedChild)) {
    ...
    // 检测是不是 ReactElement 对象
    if (isValidElement(mappedChild)) {
      mappedChild = cloneAndReplaceKey(mappedChild,
      // 保留（映射）和旧键（如果它们不同），就像traverseAllChildren用作子对象一样。。。 渣渣机翻
...
function mapIntoWithKeyPrefixInternal(children, array, prefix, func, context) {
  // 在调用 React.Children.map 时， prefix 默认传的 null, array 就是遍历之后最后返回的数组
  // escapedPrefix 目前情况下就是空字符串
  var escapedPrefix = '';
  if (prefix != null) {
   ...
   *  func : func,(就是 React.Children.map(array, function) 传入的第二个参数function)
   *  context : context,
   *  count : 0,
   * 
   * }
 ...
 */
// 计算 children 的数量，深度计算, countChildren([1,2, [3, 4]]) return 4
...
// React.Children.only, 必须传入的子元素是 ReactElement
...
// 创建上下文  https://react.docschina.org/docs/context.html,  https://reactjs.org/docs/context.html
function createContext(defaultValue, calculateChangedBits) {
  // calculateChangedBits 要么是null 要么是 function
  ...
  var context = {
    ...
    // 作为支持多个并发渲染器的变通方法, 我们将部分渲染器分为主要渲染器，其他的归为次要渲染器。
    //  我们只希望最多有两个渲染器同时存在： RN (主要) 和 Fabric (次要), React DOM (主要) 和 React ART (次要)
    // 次要渲染器会将他们的上下文值存储到单独的字段中。
...
// https://react.docschina.org/docs/forwarding-refs.html,  https://reactjs.org/docs/forwarding-refs.html
/**
 * 
 * 
...
// 获取组件名称 这块很好懂: 字符串就是html标签， function 就是组件的构造函数， switch的是内置的一些类型。
