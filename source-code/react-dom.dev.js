...
// 在ReactDOM 之前加载 react， react-dom 依赖 react
...

{
  ...
  // 校验浏览器是否支持我们实现特殊功能所需的 API
  if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
    //  fakeNode 创建一个自定义的 react 元素 假节点23333
    var fakeNode = document.createElement('react');

    // dev环境下的 invokeGuardedCallback
    var invokeGuardedCallbackDev = function (name, func, context, a, b, c, d, e, f) {
      ...
      /**
       * 跟踪用户提供的回调是否引发错误。
       * 我们在开始时将其设置为true，然后在调用函数后立即将其设置为false。
       * 如果函数错误，`didError`永远不会设为false
       * 即使浏览器不稳定且无法调用我们的全局错误处理程序，此策略仍然有效，因为它根本不依赖于错误事件
       */
      var didError = true;
      ...
      /**
       * 给我们的假事件创建一个事件处理程序，我们将使用`dispatchEvent`同步调度我们的假事件。这程序中，我们调用用户提供的回调。
       */
      var funcArgs = Array.prototype.slice.call(arguments, 3);
      function callCallback() {
        ...
        // in the stack.
        // 立即事件监听，防止嵌套的 invokeGuardedCallback 调用时不会冲突，否则，嵌套调用将触发堆栈中任何较高调用的伪事件处理程序。
      ...
      // 创建一个全局的错误事件处理程序， 我们用这个去捕获被 throw 的值
      // that was thrown. It's possible that this error handler will fire more
      // 这个错误处理可能多次触发。
      // than once; for example, if non-React code also calls `dispatchEvent`
      // 例如，如果非React代码也调用`dispatchEvent`并且该事件的处理程序抛出
      // and a handler for that event throws. We should be resilient to most of
      // 我们应适应大多数的情况。
      // those cases. Even if our error event handler fires more than once, the
      // 即使 error handler 多次触发，
      // last error event is always used. If the callback actually does error,
      // 始终使用最后一个处理事件。如果回调实际上发生错误，
      // we know that the last error event is the correct one, because it's not
      // 我们知道最后一个错误事件是正确的，
      // possible for anything else to have happened in between our callback
      // 因为在我们的回调错误和下面的`dispatchEvent`调用之后的代码之间不可能发生任何其他事件
      // erroring and the code that follows the `dispatchEvent` call below. If
      // the callback doesn't error, but the error event was fired, we know to
      // 如果回调没有错误，但错误事件被触发
      // ignore it because `didError` will be false, as described above.
      // 我们知道忽略它，因为`didError`将为false，如上所述。
      var error = void 0;
      // Use this to track whether the error event is ever called.
      // 使用didSetError来跟踪是否曾调用错误事件
      ...
      // initEvent已经过时。。。 https://developer.mozilla.org/zh-CN/docs/Web/API/Event/initEvent
      evt.initEvent(evtType, false, false);
      // 触发回调，如果运行期间出现错误，就会执行下面的错误。
      fakeNode.dispatchEvent(evt);
      
      // didError 默认为true，在触发事件的时候，如果执行顺利，didError 就会变为 false
      if (didError) {
        if (!didSetError) {
 ...
 * 事件插件的可注射排序
 */
// 这个变量存储事件插件的名称，顺序与 plugins 中对应。event anem数组
 ...
 * 以 event name 为 key 存储 config
 */
var eventNameDispatchConfigs = {};

/**
 * Mapping from registration name to plugin module
 * registration name 与 plugin module
 */
var registrationNameModules = {};

/**
 * Mapping from registration name to event name
 * registration name 与 event name 映射
 */
 ...
 * only in true.
 * 从小写注册名称映射到正确套装版本，用于在缺少事件处理程序的情况下发出警告。 仅适用于true
 * @type {Object}
 */
 ...
 * 
 * 这样可以将排序与实际插件的注入分离，这样无论包装，动态注射等，排序总是确定的。
 *
...
function injectEventPluginsByName(injectedNamesToPlugins) {
  ...
  for (var pluginName in injectedNamesToPlugins) {
    ...
    // 没有这个 plugin 或者 plugin与原有的plugin不是同一个
    if (!namesToPlugins.hasOwnProperty(pluginName) || namesToPlugins[pluginName] !== pluginModule) {
      // 两个不同的plugin， pluginName却相同，抛出错误
...
// 事件插件注册。 都是引用类型，可以改变引用类型的值。
var EventPluginRegistry = Object.freeze({
  // Ordered list of injected plugins.
  plugins: plugins,
  // Mapping from event name to dispatch config
  // event name => dispatch config
  eventNameDispatchConfigs: eventNameDispatchConfigs,
  // Mapping from registration name to plugin module
  // registration name => plugin module
  ...
  // 通过注入事件的 name 排序 传入的参数是 injectedNamesToPlugins， name(从函数中看出是plugin name) => plugin
...
function executeDispatch(event, simulated, listener, inst) {
  ...
  // inst 是 FiberNode , getNodeFromInstance 返回真实节点 inst.stateNode
...
function executeDispatchesInOrder(event, simulated) {
  ...
  // 如果父组件和子组件都绑定了点击事件，dispatchListeners 就是数组
 ...
 * 在第一个项目中累积不能为null或undefined
 * is used to conserve memory by avoiding array allocations, and thus sacrifices
 * 这用于通过避免数组分配来节省内存，从而牺牲API清洁度
 ...
 * 谨慎使用这个API， 尝试 accumulate ，更清洁
 *
 * @return {*|array<*>} An accumulation of items.
 */

// 最终返回一个数组，合并 current 和 next 貌似用于事件的？
 ...
 *
 * @param {?object} event Synthetic event to be dispatched. 被派发的合成事件
 * @param {boolean} simulated If the event is simulated (changes exn behavior) 是否是模拟事件
 * @private
 */
// 执行派发与发布？
var executeDispatchesAndRelease = function (event, simulated) {
  if (event) {
      // simulated这个参数实际并没有起到任何作用。
...
// 是否需要防止的鼠标事件 只要针对有disabled属性的表单元素
function shouldPreventMouseEvent(name, type, props) {
  switch (name) {
    ...
    // 有disabled属性，并且是 button, input, select 和 textarea
 ...
 *     当触发顶级事件时，此方法应该提取合成事件，然后将合成事件进行排队和分派
 *
 *   `eventTypes` {object}
...
function runEventsInBatch(events, simulated) {
  ...
  // executeDispatchesAndReleaseSimulated 与 executeDispatchesAndReleaseTopLevel 就是内部执行 executeDispatchesAndRelease的时候传入第2个参数不同
  // 最终调用 executeDispatch， 其中 simulated 暂时无作用 
  if (simulated) {
...
// 内部实例key node[internalInstanceKey] = FiberNode
var internalInstanceKey = '__reactInternalInstance$' + randomKey;
// 内部事件处理key node[internalEventHandlersKey] = props
var internalEventHandlersKey = '__reactEventHandlers$' + randomKey;

// 预缓存 FiberNode 传入的 node 是 dom 节点
 ...
 * 给定DOM节点，返回最接近的 ReactDOMComponent 或 ReactDOMTextComponent 实例的祖先
 */
...
// internalEventHandlersKey => props ?? 所以这个 EventHandler 实际就是指的 props ?
function updateFiberProps(node, props) {
  node[internalEventHandlersKey] = props;
}

// React DOM 组件树 ？ 啊咧 一个 tree 怎么全是存的各类方法。。
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  // ReactDOM.render = legacyRenderSubtreeIntoContainer(null, children, container, false, callback)
